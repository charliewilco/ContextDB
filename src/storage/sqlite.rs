    ...
    fn matches_context(&self, context: &serde_json::Value, filter: &ContextFilter) -> bool {
        match filter {
            ...
            ContextFilter::PathEquals(path, value) => {
                context.pointer(path) == Some(value)
            }
            ...
            ContextFilter::And(filters) => {
                filters.iter().all(|f| self.matches_context(context, f))
            }
            ContextFilter::Or(filters) => {
                filters.iter().any(|f| self.matches_context(context, f))
            }
        }
    }
...
impl StorageBackend for SqliteStorage {
    ...
    fn get(&self, id: Uuid) -> StorageResult<Entry> {
        ...
        let entry = stmt.query_row(params![id_str], |row| {
        ...
        let mut rel_stmt = self.conn.prepare(
            "SELECT to_id FROM relations WHERE from_id = ?1"
        ).map_err(|e| StorageError::Database(e.to_string()))?;
    ...
    fn query(&self, query: &Query) -> StorageResult<Vec<QueryResult>> {
        ...
        let query_results: Vec<QueryResult> = results.into_iter().map(|entry| {
            let similarity_score = query.meaning.as_ref().map(|m| {
                crate::types::cosine_similarity(&entry.meaning, &m.vector)
            });
...
mod tests {
    ...
    fn test_insert_entry_with_context() {
        ...
        let entry = create_test_entry(vec![0.1], "With context")
    ...
    fn test_get_preserves_timestamps() {
        ...
        assert!((retrieved.updated_at - original_updated).num_seconds().abs() < 1);
    }
    ...
    fn test_update_entry_context() {
        let mut storage = create_test_storage();
        let mut entry = create_test_entry(vec![0.1], "Test")
            .with_context(serde_json::json!({"version": 1}));
        storage.insert(&entry).unwrap();
    ...
    fn test_update_entry_relations() {
        ...
        let mut entry = create_test_entry(vec![0.3], "Entry")
            .add_relation(target1.id);
        storage.insert(&entry).unwrap();
    ...
    fn test_delete_entry_with_relations() {
        ...
        let entry = create_test_entry(vec![0.2], "Entry")
            .add_relation(target.id);
        storage.insert(&entry).unwrap();
    ...
    fn test_matches_context_path_equals() {
        ...
        let filter_wrong = ContextFilter::PathEquals("/status".to_string(), serde_json::json!("inactive"));
    ...
    fn test_matches_context_path_contains() {
        ...
        let filter_missing = ContextFilter::PathContains("/tags".to_string(), serde_json::json!("python"));
        assert!(!storage.matches_context(&context, &filter_missing));
    }
    ...
    fn test_matches_temporal_created_between() {
        ...
        assert!(storage.matches_temporal(
            &entry,
            &TemporalFilter::CreatedBetween(past, future)
        ));
    ...
    fn test_query_all_entries() {
        let mut storage = create_test_storage();
        storage.insert(&create_test_entry(vec![0.1], "Entry 1")).unwrap();
        storage.insert(&create_test_entry(vec![0.2], "Entry 2")).unwrap();
        storage.insert(&create_test_entry(vec![0.3], "Entry 3")).unwrap();
    ...
    fn test_query_with_limit() {
        let mut storage = create_test_storage();
        for i in 0..10 {
            storage.insert(&create_test_entry(vec![i as f32], &format!("Entry {}", i))).unwrap();
        }
    ...
    fn test_query_with_limit_zero() {
        let mut storage = create_test_storage();
        storage.insert(&create_test_entry(vec![0.1], "Entry")).unwrap();
    ...
    fn test_query_by_expression_equals() {
        let mut storage = create_test_storage();
        storage.insert(&create_test_entry(vec![0.1], "Target")).unwrap();
        storage.insert(&create_test_entry(vec![0.2], "Other")).unwrap();

        let query = Query::new()
            .with_expression(ExpressionFilter::Equals("Target".to_string()));
        let results = storage.query(&query).unwrap();
    ...
    fn test_query_by_expression_contains() {
        let mut storage = create_test_storage();
        storage.insert(&create_test_entry(vec![0.1], "Hello World")).unwrap();
        storage.insert(&create_test_entry(vec![0.2], "World Hello")).unwrap();
        storage.insert(&create_test_entry(vec![0.3], "Goodbye")).unwrap();

        let query = Query::new()
            .with_expression(ExpressionFilter::Contains("world".to_string()));
        let results = storage.query(&query).unwrap();
    ...
    fn test_query_by_meaning_similarity() {
        let mut storage = create_test_storage();
        storage.insert(&create_test_entry(vec![1.0, 0.0, 0.0], "X axis")).unwrap();
        storage.insert(&create_test_entry(vec![0.0, 1.0, 0.0], "Y axis")).unwrap();
        storage.insert(&create_test_entry(vec![0.0, 0.0, 1.0], "Z axis")).unwrap();

        // Query for vectors similar to X axis
        let query = Query::new()
            .with_meaning(vec![1.0, 0.0, 0.0], Some(0.9));
        let results = storage.query(&query).unwrap();
    ...
    fn test_query_by_meaning_top_k() {
        let mut storage = create_test_storage();
        storage.insert(&create_test_entry(vec![1.0, 0.0], "Very similar")).unwrap();
        storage.insert(&create_test_entry(vec![0.9, 0.1], "Similar")).unwrap();
        storage.insert(&create_test_entry(vec![0.0, 1.0], "Different")).unwrap();
    ...
    fn test_query_by_context() {
        let mut storage = create_test_storage();
        storage.insert(&create_test_entry(vec![0.1], "Entry 1")
            .with_context(serde_json::json!({"type": "user"}))).unwrap();
        storage.insert(&create_test_entry(vec![0.2], "Entry 2")
            .with_context(serde_json::json!({"type": "system"}))).unwrap();

        let query = Query::new()
            .with_context(ContextFilter::PathEquals("/type".to_string(), serde_json::json!("user")));
        let results = storage.query(&query).unwrap();
    ...
    fn test_query_combined_filters() {
        let mut storage = create_test_storage();
        storage.insert(&create_test_entry(vec![1.0, 0.0], "Hello World")
            .with_context(serde_json::json!({"type": "greeting"}))).unwrap();
        storage.insert(&create_test_entry(vec![1.0, 0.0], "Hello There")
            .with_context(serde_json::json!({"type": "greeting"}))).unwrap();
        storage.insert(&create_test_entry(vec![0.0, 1.0], "Hello Different")
            .with_context(serde_json::json!({"type": "greeting"}))).unwrap();
        storage.insert(&create_test_entry(vec![1.0, 0.0], "Goodbye World")
            .with_context(serde_json::json!({"type": "farewell"}))).unwrap();

        // Semantic + Expression + Context
        let query = Query::new()
            .with_meaning(vec![1.0, 0.0], Some(0.9))
            .with_expression(ExpressionFilter::Contains("hello".to_string()))
            .with_context(ContextFilter::PathEquals("/type".to_string(), serde_json::json!("greeting")));
    ...
    fn test_query_with_explanation() {
        let mut storage = create_test_storage();
        storage.insert(&create_test_entry(vec![1.0, 0.0], "Test")).unwrap();
    ...
    fn test_query_no_matches() {
        ...
        let query = Query::new()
            .with_expression(ExpressionFilter::Equals("Nonexistent".to_string()));
        let results = storage.query(&query).unwrap();
