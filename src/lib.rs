...
pub use types::{Entry, cosine_similarity};
pub use query::{
    Query, QueryResult, MeaningFilter, ExpressionFilter, 
    ContextFilter, RelationFilter, TemporalFilter
};
pub use storage::{StorageBackend, StorageError, StorageResult, SqliteStorage};
...
mod tests {
    ...
    fn test_semantic_query_ordering() {
        ...
        db.insert(&Entry::new(vec![1.0, 0.0], "Exact match".to_string())).unwrap();
        db.insert(&Entry::new(vec![0.7, 0.7], "Partial match".to_string())).unwrap();
        db.insert(&Entry::new(vec![0.0, 1.0], "No match".to_string())).unwrap();
    ...
    fn test_text_query() {
        ...
        db.insert(&Entry::new(vec![0.1, 0.2], "loves onions".to_string())).unwrap();
        db.insert(&Entry::new(vec![0.1, 0.2], "hates red onions".to_string())).unwrap();
        db.insert(&Entry::new(vec![0.1, 0.2], "prefers garlic".to_string())).unwrap();
    ...
    fn test_text_query_equals() {
        ...
        db.insert(&Entry::new(vec![0.1], "exact match".to_string())).unwrap();
        db.insert(&Entry::new(vec![0.2], "Exact Match".to_string())).unwrap();
        db.insert(&Entry::new(vec![0.3], "exact match with more".to_string())).unwrap();

        let query = Query::new()
            .with_expression(ExpressionFilter::Equals("exact match".to_string()));
    ...
    fn test_text_query_starts_with() {
        ...
        db.insert(&Entry::new(vec![0.1], "Hello World".to_string())).unwrap();
        db.insert(&Entry::new(vec![0.2], "Hello There".to_string())).unwrap();
        db.insert(&Entry::new(vec![0.3], "World Hello".to_string())).unwrap();

        let query = Query::new()
            .with_expression(ExpressionFilter::StartsWith("Hello".to_string()));
    ...
    fn test_context_query() {
        ...
        db.insert(&Entry::new(vec![0.1], "User 1".to_string())
            .with_context(serde_json::json!({"role": "admin"}))).unwrap();
        db.insert(&Entry::new(vec![0.2], "User 2".to_string())
            .with_context(serde_json::json!({"role": "user"}))).unwrap();
        db.insert(&Entry::new(vec![0.3], "User 3".to_string())
            .with_context(serde_json::json!({"role": "admin"}))).unwrap();

        let query = Query::new()
            .with_context(ContextFilter::PathEquals("/role".to_string(), serde_json::json!("admin")));
    ...
    fn test_context_query_nested() {
        ...
        let query = Query::new()
            .with_context(ContextFilter::PathEquals("/metadata/active".to_string(), serde_json::json!(true)));
    ...
    fn test_context_query_path_contains() {
        ...
        db.insert(&Entry::new(vec![0.1], "Tagged".to_string())
            .with_context(serde_json::json!({"tags": ["important", "urgent"]}))).unwrap();
        db.insert(&Entry::new(vec![0.2], "Not tagged".to_string())
            .with_context(serde_json::json!({"tags": ["normal"]}))).unwrap();

        let query = Query::new()
            .with_context(ContextFilter::PathContains("/tags".to_string(), serde_json::json!("urgent")));
    ...
    fn test_temporal_query_created_after() {
        ...
        db.insert(&Entry::new(vec![0.1], "Recent entry".to_string())).unwrap();
    ...
    fn test_temporal_query_created_before_filters_out() {
        ...
        db.insert(&Entry::new(vec![0.1], "Entry".to_string())).unwrap();

        let past = Utc.with_ymd_and_hms(2020, 1, 1, 0, 0, 0).unwrap();
        let query = Query::new()
            .with_temporal(TemporalFilter::CreatedBefore(past));
    ...
    fn test_combined_semantic_and_text_query() {
        ...
        db.insert(&Entry::new(vec![1.0, 0.0], "Hello semantic".to_string())).unwrap();
        db.insert(&Entry::new(vec![1.0, 0.0], "Goodbye semantic".to_string())).unwrap();
        db.insert(&Entry::new(vec![0.0, 1.0], "Hello different".to_string())).unwrap();
    ...
    fn test_combined_all_filters() {
        ...
        db.insert(&Entry::new(vec![1.0, 0.0], "Target entry".to_string())
            .with_context(serde_json::json!({"type": "target"}))).unwrap();

        // Wrong vector
        db.insert(&Entry::new(vec![0.0, 1.0], "Target entry".to_string())
            .with_context(serde_json::json!({"type": "target"}))).unwrap();

        // Wrong text
        db.insert(&Entry::new(vec![1.0, 0.0], "Other entry".to_string())
            .with_context(serde_json::json!({"type": "target"}))).unwrap();

        // Wrong context
        db.insert(&Entry::new(vec![1.0, 0.0], "Target entry".to_string())
            .with_context(serde_json::json!({"type": "other"}))).unwrap();

        let query = Query::new()
            .with_meaning(vec![1.0, 0.0], Some(0.9))
            .with_expression(ExpressionFilter::Contains("target".to_string()))
            .with_context(ContextFilter::PathEquals("/type".to_string(), serde_json::json!("target")))
            .with_temporal(TemporalFilter::CreatedAfter(past));
    ...
    fn test_query_with_limit() {
        ...
        for i in 0..20 {
            db.insert(&Entry::new(vec![i as f32], format!("Entry {}", i))).unwrap();
        }
    ...
    fn test_query_with_explanation() {
        ...
        db.insert(&Entry::new(vec![1.0, 0.0], "Test".to_string())).unwrap();
    ...
    fn test_empty_query_returns_all() {
        ...
        db.insert(&Entry::new(vec![0.1], "Entry 1".to_string())).unwrap();
        db.insert(&Entry::new(vec![0.2], "Entry 2".to_string())).unwrap();
        db.insert(&Entry::new(vec![0.3], "Entry 3".to_string())).unwrap();
