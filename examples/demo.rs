use contextdb::{ContextDB, Entry, Query, ExpressionFilter, TemporalFilter, ContextFilter};
use chrono::Utc;
use serde_json::json;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let entries = vec![
        ...
        Entry::new(
            vec![0.4, 0.6, 0.5],
            "User allergic to peanuts".to_string(),
        ).with_context(json!({
            "category": "dietary",
            "specificity": "allergy",
            "allergen": "peanuts",
    ...
    let severity_query = Query::new()
        .with_context(ContextFilter::PathEquals("/severity".to_string(), json!("high")));
    ...
    let hybrid_query = Query::new()
        .with_context(ContextFilter::PathEquals("/specificity".to_string(), json!("item-level")))
    ...
    let work_query = Query::new()
        .with_context(ContextFilter::PathEquals("/category".to_string(), json!("work")));
    ...
    let dietary_count = db.query(&Query::new()
        .with_context(ContextFilter::PathEquals("/category".to_string(), json!("dietary"))))?
        .len();
    
    let work_count = db.query(&Query::new()
        .with_context(ContextFilter::PathEquals("/category".to_string(), json!("work"))))?
        .len();
